...
//    cv::waitKey(0);
    ...
    // find matches
    cout << "wait for matching..." << endl;
    vector<cv::DMatch> matches, matches2;
    chrono::steady_clock::time_point t1 = chrono::steady_clock::now();
    bfMatch(descriptors, descriptors2, matches);
    chrono::steady_clock::time_point t2 = chrono::steady_clock::now();
    chrono::duration<double> time_used = chrono::duration_cast<chrono::duration<double>>( t2-t1 );
    cout << "matches: " << matches.size() << endl;
    cout << "matching time cost: " << time_used.count() << " seconds. " << endl;
...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
	...
	// START YOUR CODE HERE (~7 lines)
        int m_10 = 0, m_01 = 0;
        for (int u = -half_patch_size; u <= half_patch_size-1; ++u) {       // col
            for (int v = -half_patch_size; v <= half_patch_size-1; ++v) {   // row
                int u1 = cvRound(kp.pt.x) + u, v1 = cvRound(kp.pt.y) + v;   // u1, kp.pt.x(col); v1,kp.pt.y(row)
                if (u1 >= 0 && u1 < image.cols && v1 >= 0 && v1 < image.rows) {
                    m_10 += u  * image.at<uchar>(v1, u1);   // image.at<uchar>(y, x) = image.at<cv::Point2i>(Point2i(x, y))
                    m_01 += v  * image.at<uchar>(v1, u1);
                } else continue;
            }
        }
        kp.angle = cv::fastAtan2((float)m_01, (float)m_10); // atan2(m_01, m_10)*180/pi
    // END YOUR CODE HERE
    }
    return;
}
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
    for (auto &kp : keypoints) {
        ...
        for (int i = 0; i < 256; i++) {
            ...
            int u_p = cvRound( up*cos(angle) - vp*sin(angle) );
            int v_p = cvRound( up*sin(angle) + vp*cos(angle) );
            int u_q = cvRound( uq*cos(angle) - vq*sin(angle) );
            int v_q = cvRound( uq*sin(angle) + vq*cos(angle) ); // 先转后加
            if (kp.pt.x+u_p < 0 || kp.pt.x+u_p > image.cols-1 || kp.pt.x+u_q < 0 || kp.pt.x+u_q > image.cols-1 || kp.pt.y+v_p < 0 || kp.pt.y+v_p > image.rows-1 || kp.pt.y+v_q < 0 || kp.pt.y+v_q > image.rows-1) {
                d.clear();  // if kp goes outside, set d.clear()
                break;      // next keypoint
            } else if ( image.at<uchar>(cvRound(kp.pt.y)+v_p, cvRound(kp.pt.x)+u_p) <= image.at<uchar>(cvRound(kp.pt.y)+v_q, cvRound(kp.pt.x)+u_q) ) {
                d[i] = 1;
            }
        }
	    // END YOUR CODE HERE
...
void bfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
    ...
    vector<int> ham_dis, ham_dis_index;
    cv::DMatch match;
    for (int i=0; i<desc1.size(); i++) {
        ...
        for (int j=0; j<desc2.size(); j++) {
            ...
            int d = 0;
            for (int k=0; k<256; k++) {
                if (desc1[i][k] != desc2[j][k]) d++;    // 计算汉明距离
            }
            ham_dis.push_back(d);       // desc1中的第i个有效描述子与desc2中所有有效描述子的汉明距离
            ham_dis_index.push_back(j); // 对应的索引
        }

        vector<int>::iterator d_min = min_element(ham_dis.begin(), ham_dis.end());  // 最小值
        int n = distance(ham_dis.begin(), d_min);   // 最小值索引位置
        if (*d_min <= d_max) {
            match.queryIdx = i;
            match.trainIdx = ham_dis_index[n];
